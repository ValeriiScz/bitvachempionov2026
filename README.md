...
5. ⚠️ **Брать ОФИЦИАЛЬНУЮ таблицу mafgame, НЕ пересчитывать самим.** Урок (v10.1 Кёльн-702, уточнён в v14.1 на CEC-694 и ЧЕ-634): в официальную таблицу баллы финальных игр входят с **коэффициентом ×1.3, причём на ВСЁ** (WPTS, допы, штрафы, лучший ход, Ci); полуфинал — без коэффициента. В протоколах game_results лежат сырые баллы, поэтому live-таблица финала занижала Σ. Решение: `tournament_{id}.json.final_coef` (напр. 1.3) — live.js (v2.1) умножает баллы стадии «Финал» на лету, а при заморозке коэффициент вшивается в games_{id}_current.json. Сверка 694: квалификация 30/30 и финал 10/10 — 1:1. Для завершённых источник истины — всё равно таблица mafgame.
