...
json.dump({'T':T,'S':3000,'thr':thr.tolist(),'rows':[{'uid':u,'nick':g.NICK[u],'pos':midpos[u],'played':float(res['played'][pool.index(u)]),'p12':float(res['p12'][pool.index(u)])} for s,u in mid[:60]]},open(OUTP('forecast2026.json'),'w'),ensure_ascii=False)
